...
Friend Module 图站检索
	...
	Public Async Function IQDB查询(图片路径 As String) As Task(Of IEnumerable(Of 搜索结果))
		Return From 结果 As Match In (Await IQDB低级查询(图片路径)).Matches Select New 搜索结果(结果)
	End Function

	Public Function IQDB低级查询(图片路径 As String) As Task(Of SearchResult)
		Dim 文件流 As FileStream = File.OpenRead(图片路径)
		IQDB低级查询 = IQDB客户端.SearchFile(File.OpenRead(图片路径))
		文件流.Close()
	End Function
	...
	Public Async Function 从网页保存原图(网页URL As String) As Task(Of 保存结果)
		...
		If 网页URL.Contains("konachan") Then
			...
			If HTML集合.Any Then
				...
				Try
					成功 = Await 尝试入库(Await HTTP客户端.GetByteArrayAsync(文件URL), Path.GetFileName(文件URL))
	...
	Public Async Function 搜索并筛选原图(路径 As String) As Task(Of 保存结果)
		Dim 原始结果 As SearchResult = Await IQDB低级查询(路径)
		...
		Dim 结果列表 As IEnumerable(Of Match)
		If 原图尺寸.Width >= 原图尺寸.Height Then
			结果列表 = From 结果 As Match In Match列表 Where 结果.Resolution.Width >= 结果.Resolution.Height
		Else
			结果列表 = From 结果 As Match In Match列表 Where 结果.Resolution.Width < 结果.Resolution.Height
		End If
