...
Module 图库哈希
	...
	ReadOnly SHA256计算器 As SHA256 = SHA256.Create
	Private 哈希流 As FileStream
	Const 文件名 As String = "哈希表.sha256"
	Private Function 新建哈希表(哈希表路径 As String) As Task
		Return Task.Run(Sub()
							哈希表 = (From 路径 In Directory.GetFiles(设置项.下载保存路径, "*.jpg").Concat(Directory.GetFiles(设置项.下载保存路径, "*.png")).AsParallel Select SHA256计算器.ComputeHash(File.OpenRead(路径))).ToList
	...
	Async Function 尝试入库(新图 As Byte(), 文件名 As String) As Task(Of Boolean)
		Dim 新图哈希 As Byte() = SHA256计算器.ComputeHash(新图)
		Await 哈希任务
		If 哈希表.Contains(新图哈希, 哈希比较器) Then
			Return False
		Else
			哈希表.Add(新图哈希)
			哈希流.Write(新图哈希)
