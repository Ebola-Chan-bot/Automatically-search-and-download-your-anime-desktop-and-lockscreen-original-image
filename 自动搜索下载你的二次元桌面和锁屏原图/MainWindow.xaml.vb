...
Class MainWindow
	...
	Sub New()
		...
		扩展名一律设为PNG.IsChecked = 设置项.扩展名一律设为PNG
	...
	Private Sub 每隔时间值_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles 每隔时间值.SelectionChanged
		设置项.每隔时间值 = If(每隔时间值.SelectedIndex = -1, 0, 每隔时间值.SelectedIndex)
	End Sub
	...
	Private Sub 桌面_浏览_Click(sender As Object, e As RoutedEventArgs) Handles 桌面_浏览.Click
		Static 桌面浏览对话框 As New FolderBrowserDialog With {.SelectedPath = 设置项.当前桌面路径}
		If 桌面浏览对话框.ShowDialog = Forms.DialogResult.OK Then
			设置桌面路径(桌面浏览对话框.SelectedPath)
		End If
	End Sub

	Private Sub 锁屏_浏览_Click(sender As Object, e As RoutedEventArgs) Handles 锁屏_浏览.Click
		Static 锁屏浏览对话框 As New FolderBrowserDialog With {.SelectedPath = 设置项.当前锁屏路径}
		If 锁屏浏览对话框.ShowDialog = Forms.DialogResult.OK Then
			设置锁屏路径(锁屏浏览对话框.SelectedPath)
		End If
	End Sub
	...
	Private Sub 浏览下载路径_Click(sender As Object, e As RoutedEventArgs) Handles 浏览下载路径.Click
		Static 下载浏览对话框 As New FolderBrowserDialog With {.SelectedPath = 设置项.下载保存路径}
		If 下载浏览对话框.ShowDialog = Forms.DialogResult.OK Then
			设置项.下载保存路径 = 下载浏览对话框.SelectedPath
			下载保存路径.Text = 下载浏览对话框.SelectedPath
		End If
	End Sub
	...
	Private Sub 扩展名一律设为PNG_Checked(sender As Object, e As RoutedEventArgs) Handles 扩展名一律设为PNG.Checked, 扩展名一律设为PNG.Unchecked
		设置项.扩展名一律设为PNG = 扩展名一律设为PNG.IsChecked
	End Sub
